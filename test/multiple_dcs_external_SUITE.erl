...
-module(multiple_dcs_external_SUITE).
...
simple_replication_test(Config) ->
    lager:info("Starting external test"),
    Clusters = proplists:get_value(clusters, Config),
    [Node1, Node2, Node3 | _Nodes] =  [ hd(Cluster)|| Cluster <- Clusters ],

    Key = ex_simple_replication_test_dc,
    Type = antidote_crdt_counter_pn,

    Properties = [{type_tx, external}, {error_sync, 0}],

    update_counters(Node1, [Key], [1], ignore, static, Properties),
    update_counters(Node1, [Key], [1], ignore, static, Properties),
    {ok, CommitTime} = update_counters(Node1, [Key], [1], ignore, static, Properties),
    ...
    {ok, CommitTime2} = update_counters(Node2, [Key], [1], CommitTime, static, Properties),

    {ok, CommitTime3} = update_counters(Node3, [Key], [1], CommitTime2, static, Properties),
    ...
    lager:info("Done external test"),
    pass.

parallel_writes_test(Config) ->
    Clusters = proplists:get_value(clusters, Config),
    [Node1, Node2, Node3 | _Nodes] =  [ hd(Cluster)|| Cluster <- Clusters ],
    Key = ex_parallel_writes_test,
    Type = antidote_crdt_counter_pn,
    Pid = self(),
...
multiple_writes(Node, Key, ReplyTo) ->
    Properties = [],
    update_counters(Node, [Key], [1], ignore, static, Properties),
    update_counters(Node, [Key], [1], ignore, static, Properties),
    update_counters(Node, [Key], [1], ignore, static, Properties),
    update_counters(Node, [Key], [1], ignore, static, Properties),
    {ok, CommitTime} = update_counters(Node, [Key], [1], ignore, static, Properties),
...
failure_test(Config) ->
    ...
    case rpc:call(Node1, application, get_env, [antidote, enable_logging]) of
        ...
        _ ->
            Type = antidote_crdt_counter_pn,
            Key = ex_multiplde_dc_failure_test,
...
blocking_test(Config) ->
    ...
    {ok, CommitTime1} = update_counters(Node1, [Key], [1], ignore, static, Properties),
    {ok, CommitTime2} = update_counters(Node2, [Key], [1], ignore, static, Properties),
...
check_read(Node, Objects, Expected, Clock, TxId) ->
    case TxId of
        static ->
            ...
            lager:info("I expect: ~p, the result was: ~p", [Expected, Res]),
...
update_counters(Node, Keys, IncValues, Clock, TxId, Properties) ->
    ...
    case TxId of
        static ->
            {ok, CT} = rpc:call(Node, cure, update_objects, [Clock, Properties, Updates]),
            {ok, CT};
        _->
